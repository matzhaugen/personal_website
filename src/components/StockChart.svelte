<script>
	...
	minx = points[0].dateNumber;
	maxx = points[points.length - 1].dateNumber;

	for (let i = 0; i < points.length; i += 1) {
		const point = points[i];

		if (point.adjClose < miny) {
			miny = point.adjClose;
		}

		if (point.adjClose > maxy) {
			maxy = point.adjClose;
			highest = point;
		}
	}
...
<div class="chart">
	<Pancake.Chart x1={minx} x2={maxx} y1={miny} y2={maxy}>
		...
		<Pancake.Grid horizontal count={5} let:value let:last>
			<div class="grid-line horizontal"><span>{value} {last ? 'units' : ''}</span></div>
		</Pancake.Grid>
		...
		<Pancake.Svg>
			<Pancake.SvgScatterplot data={points} x="{d => d.dateNumber}" y="{d => d.adjClose}" let:d>
				<path class="adjClose scatter" {d}/>
			</Pancake.SvgScatterplot>

			<Pancake.SvgLine data={points} x="{d => d.dateNumber}" y="{d => d.adjClose}" let:d>
				<path class="adjClose" {d}/>
			</Pancake.SvgLine>

			<!-- <Pancake.SvgLine data={points} x="{d => d.dateNumber}" y="{d => d.trend}" let:d>
				<path class="trend" {d}/>
			</Pancake.SvgLine> -->
		</Pancake.Svg>
		...
		<!-- <Pancake.Point x={highest.dateNumber} y={highest.adjClose}>
			<div class="annotation" style="position: absolute; right: 0.5em; top: -0.5em; white-space: nowrap; line-height: 1; color: #666;">
				{highest.adjClose} parts per million (ppm) &rarr;
			</div>
		</Pancake.Point> -->

		<Pancake.Quadtree data={points} x="{d => d.dateNumber}" y="{d => d.adjClose}" let:closest>
			{#if closest}
				<Pancake.Point x={closest.dateNumber} y={closest.adjClose} let:d>
					<div class="focus"></div>
					<div class="tooltip" style="transform: translate(-{pc(closest.dateNumber)}%,0)">
						<strong>${closest.adjClose.toPrecision(6)}</strong>
						<span>{format(closest.dateNumber)}</span>
					</div>
				</Pancake.Point>
			{/if}
		</Pancake.Quadtree>
